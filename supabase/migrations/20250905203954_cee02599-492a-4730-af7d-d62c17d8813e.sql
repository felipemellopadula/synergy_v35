...
-- Add 'admin' to subscription_type enum if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'admin' AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'subscription_type')) THEN
        ALTER TYPE subscription_type ADD VALUE 'admin';
    END IF;
END $$;
