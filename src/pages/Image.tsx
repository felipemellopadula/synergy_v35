...
// As configurações que definimos anteriormente continuam corretas.
...
const MODEL_CONFIG = {
  model: MODELS[0].id,
  cfgScale: 7.0, // Ajustado para um valor padrão mais comum
  sampler: "DPM++ 2M Karras", // Mantido conforme documentação
...
const ImagePage = () => {
  ...
  const generate = async () => {
    ...
    try {
      ...
      // O `body` agora é um array de tarefas, exatamente como a documentação da Runware exige.
      // A sua função Supabase deve ser simples: apenas pegar este `body` e passar para a Runware.
      // Ela irá inserir a API Key na primeira tarefa.
      ...
      const inferenceTask: any = {
        ...
        CFGScale: MODEL_CONFIG.cfgScale,
        scheduler: MODEL_CONFIG.sampler,
        numberResults: 1,
        outputType: ["dataURI"],
        outputFormat: "PNG",
      };
      ...
      if (inputImageBase64) {
        inferenceTask.image = inputImageBase64;
      }
      
      // Criamos a estrutura de array de tarefas.
      // A primeira tarefa (autenticação) será preenchida pela sua função Supabase.
      const tasks = [
        {
          "taskType": "authentication",
          // A API Key será adicionada no backend (Supabase Function)
        },
        inferenceTask // A segunda tarefa é a de geração da imagem
      ];

      // Enviamos o array completo no corpo da requisição.
      const { data, error } = await supabase.functions.invoke('generate-image', {
        body: { tasks } // A função Supabase deve esperar um objeto com a chave "tasks"
      });

      if (error) throw error;
      
      // A resposta da Runware também vem em um array. Precisamos encontrar o resultado da inferência.
      const resultData = data.find((d: any) => d.taskType === 'imageInference' && d.imageDataURI);
      if (!resultData || !resultData.imageDataURI) {
        // Se não encontrar, verificar se há um erro retornado pela Runware
        const errorResult = data.find((d: any) => d.status === 'failed');
        if (errorResult) {
            throw new Error(`Falha na API da Runware: ${errorResult.taskError}`);
        }
        throw new Error('A API não retornou uma imagem. Verifique o log da função Supabase.');
      }

      const imageDataURI = resultData.imageDataURI as string;
